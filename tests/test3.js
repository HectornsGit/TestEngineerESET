...
//---------------------------------------------------------------------------------------------
// Test 3. Set collection profile to "Threat detection".
// This test only works if Collection profile is set to Default
//---------------------------------------------------------------------------------------------
const testSetCollectionProfileToThreatDetection = async (
  alreadyTested = true
) => {
  try {
    ...
    if (collectionProfileSelector) {
      // 1. changes "collection profile" value to "Threat detection".
      await mouse.move(straightTo(centerOf(collectionProfileSelector)));
      await mouse.click(Button.LEFT);
      await mouse.move(down(35));
      await mouse.click(Button.LEFT);
...
testSetCollectionProfileToThreatDetection(alreadyTested);
